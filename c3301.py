...
class PuzzleMaster:
    ...
    def create_new_puzzle(self, difficulty=1):
        """
        Generates a new puzzle package. This is the only public method.
        It returns the puzzle, a clue, and a HASH of the solution.
        The plaintext solution is never returned, simulating amnesia.
        """
        # 1. Generate the puzzle's core components
        solution = random.choice(self.word_list)
        solution = f"{solution}{random.randint(100, 999)}" # Add salt
        shift_key = random.randint(3, 24)

        # 2. Create the puzzle and clue
        ...
        # 3. Create the hash commitment of the solution
        solution_hash = hashlib.sha256(solution.encode()).hexdigest()

        # 4. "Forget" the solution and return the package
        return {
            "puzzle": puzzle,
            "clue": clue,
